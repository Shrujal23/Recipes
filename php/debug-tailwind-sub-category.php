...
<section class="page-header shadow bg-img bg-overlay container-fluid my-5">
        <div class="row h-100 align-items-center">
            ...
            <div class="search-bar col-12 col-md-5 col-lg-4 px-3 px-sm-5 px-md-4">
                <form action="debug-tailwind-sub-category.php" method="GET">
                    <div class="input-group">
                        <input type="search" name="query" class="form-control" placeholder="Search Recipes" required>
        ...
        <section class="sub-category" id="punjabi">
            <div class="row">
                <?php 
                    // Debugging output
                    if (isset($_GET['query'])) {
                        echo "<p>Search Query: " . htmlspecialchars($_GET['query']) . "</p>"; // Debugging line
                        $searchQuery = mysqli_real_escape_string($conn, $_GET['query']);
                        $r = mysqli_query($conn, "SELECT * FROM posts WHERE title LIKE '%$searchQuery%' ORDER BY id desc");
                    } else {
